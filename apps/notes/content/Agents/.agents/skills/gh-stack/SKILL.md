...
metadata:
  author: github
  version: "0.1.0"
date: "2026-08-12T03:42:29.000Z"
created: "2026-08-12T03:42:29.000Z"
includeInRss: false
---
...
## Hub: PR descriptions (`pr-description`)

Local hub note (not upstream). After `gh stack submit --auto`, do not leave auto-generated bodies
when the user wants review-ready text.

1. `gh stack view --json`
2. Per open PR (bottom → top): check out the layer, draft with hub skill **`pr-description`**
   against the **stack parent** diff, then `gh pr edit <n> --body-file …` (and `--title` if needed).
3. Keep each body scoped to that layer only.
