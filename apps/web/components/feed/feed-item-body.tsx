...
export default function FeedItemBody({ body, bodyFormat, codeLanguage, className }: FeedBodyProps) {
  const normalizedBody = body?.trim()
  if (!normalizedBody) return null

  if (bodyFormat === "code") {
    return <CodeBlock code={normalizedBody} language={codeLanguage} />
  }
  ...
  if (bodyFormat === "html") {
    return (
      <div
        ...
        dangerouslySetInnerHTML={{ __html: sanitizeHtml(normalizedBody) }}
      />
    )
  }

  return <p className={className}>{normalizedBody}</p>
}
